...
<%
    ...
    try {
        ...
        // **新增：檢查是否為自己的書籍**
        String checkOwnerSql = "SELECT sellerId FROM bookListings WHERE listingId = ?";
        PreparedStatement checkOwnerStmt = con.prepareStatement(checkOwnerSql);
        checkOwnerStmt.setInt(1, Integer.parseInt(bookId));
        ResultSet ownerRs = checkOwnerStmt.executeQuery();
        
        if (ownerRs.next()) {
            String sellerId = ownerRs.getString("sellerId");
            if (sellerId.equals(userId)) {
                out.print("{\"success\": false, \"message\": \"無法收藏自己上架的書籍\"}");
                ownerRs.close();
